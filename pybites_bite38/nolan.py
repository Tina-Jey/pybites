...
def get_movies():
    ...
    movie_titles = []
    for mymovie in tree.iter("movie"):
        name = mymovie.get("title")
...
def get_movie_longest_runtime():
    ...
    for movie_element in tree.iter("movie"):
        title = movie_element.get("title")
        year = movie_element.get("year")
        runtime = movie_element.get("runtime")
        # print(f"Movie: {title}, Year: {year}, Runtime: {runtime}")
        
        # extracting digits from runtime
        res = ''.join(char for char in runtime if char.isdigit())
        runtime_value = int(res) if res else 0
        
        # extraxting the longest movie:
        if runtime_value > min_runtime:
            min_runtime = runtime_value
            max_runtime_movie = title
